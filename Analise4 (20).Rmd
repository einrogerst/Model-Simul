---
title: "Análise da fila de atendimento (restrita ao horário de pico 20h)"
output: pdf_document
---
...
#filtra apenas as chegadas occoridas entre 20:00:00 e 20:59:59
data <- data[format(data$arrivalTimestamp, "%H")=="20", ]
write.csv(data, file="data20.csv", row.names = F)
...
ggsave(filename = "chegadas120.png", plot = chegadas1)

write.csv(data[,"timediff"], file="chegadasFiltro20.csv")
write.csv(data[format(data$arrivalTimestamp, "%y-%m-%d") %in% highDays,"timediff"], file="chegadasFiltro20high.csv")
...
chegadas2 <- ggplot(data, aes(timediff)) +
  ...
  xlab("Intervalo em segundos") +
  scale_y_continuous(labels = percent_format())
ggsave(filename = "chegadas220.png", plot = chegadas2)
...
chegadas3 <- 
  ...
  xlab("Horas do dia")
ggsave(filename = "chegadas320.png", plot = chegadas3)
...
ggsave(filename = "chegadas420.png", plot = chegadas4)
```
...
tempo_fila_1 <- 
  ...
  scale_y_continuous(labels = percent_format())
ggsave(filename = "tempo_fila_120.png", plot = tempo_fila_1)
```
...
ggsave(filename = "duracao_atend20.png", plot = duracao_atend)

write.csv(data[data$validServTime, "servDuration"], file="duracao_atend20.csv")
write.csv(data[data$validServTime & (format(data$arrivalTimestamp, "%y-%m-%d") %in% highDays),"servDuration"], file="duracao_atend20high.csv")
